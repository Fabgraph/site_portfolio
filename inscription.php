...
if (!empty($_POST)) {  // si le formulaire est soumis
    ...
    if (empty($contenu)) {  // si $contenu est vide, c'est qu'il n'y a pas d'erreur
        ...
        if ($membre->rowCount() > 0) {  // si la requête retourne 1 ou plusieurs résultats c'est que le pseudo existe en BDD
            ...
            executeRequete("INSERT INTO t_utilisateurs (prenom, nom, email, telephone, portable, mdp, pseudo, age, anniversaire, civilite, adresse, code_postal, ville, commentaire, statut) VALUES (:prenom, :nom, :email, :telephone, :portable, :mdp, :pseudo, :age, :anniversaire, :civilite, :adresse, :code_postal, :ville, :commentaire, 0)", 
